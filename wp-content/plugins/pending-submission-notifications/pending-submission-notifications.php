...
if ($new_status == 'pending' && user_can($post->post_author, 'edit_posts') && !user_can($post->post_author, 'publish_posts')) {
	...
	$subject = 'Nueva actividad pendiente de revisión: "' . $post->post_title . '"';
	$message = 'Una actividad espera revisión.';
	$message .= "\r\n\r\n";
	$message .= "Autor: $username->user_login\r\n";
	$message .= "Título: $post->post_title\r\n";
	$message .= "Autor de últma edición: $username_last_edit\r\n";
	$message .= "Fecha de últma edición: $post->post_modified";
	$message .= "\r\n\r\n";
	$message .= "Editar la actividad: $edit_link\r\n";
	$message .= "Previsualizar: $preview_link";
...
// Notifiy Contributor that Admin has published their post
    ...
    $username = get_userdata($post->post_author);
    $url = get_permalink($post->ID);
	$subject = "Su actividad ha sido publicada:" . " " . $post->post_title;
	$message = '"' . $post->post_title . '"' . " ha sido publicada en el Portal. \r\n";
	$message .= $url;
